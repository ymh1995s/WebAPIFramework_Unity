...
**원칙:** 사용자가 이미 승인한 파일 목록은 재확인하지 않는다. 새 파일·민감 파일만 재확인한다.

**재확인 생략 조건 (즉시 구현 시작):**
오케스트레이터가 호출 프롬프트에 다음을 명시한 경우, 최초 명시된 파일 목록은 추가 확인 없이 구현한다.
- 사용자가 직전 메시지에서 명시적 승인(`진행`/`승인`/`go`/`OK` 등)을 표현
- 또는 QA 반려 후 재호출(반려 사유 명시, 기존 승인 파일 범위 내 수정)

이 경우 "[filepath]에 작성해도 될까요?" 같은 재질문을 하지 말 것. 곧바로 작성한다.

**재확인 필수 (단일/다중 모두 사용자에게 별도 확인):**
승인 여부와 무관하게, 아래는 항상 사용자에게 명시적으로 묻는다.
- 최초 명시된 파일 목록 **외**의 신규/수정 파일
...
**승인 정보가 누락된 경우:**
오케스트레이터 프롬프트에 사용자 승인 표현 또는 QA 루프 재호출 표시가 없으면, 종전대로 파일 목록을 보여주고 승인을 받는다.
