...
public static class PopupService
{
    ...
    /// <summary>
    /// 화면 하단에 잠깐 표시되었다가 자동으로 사라지는 토스트 메시지를 표시한다.
    /// API 성공/완료 피드백 등 간단한 알림에 사용한다.
    /// </summary>
    /// <param name="message">표시할 메시지</param>
    /// <param name="duration">표시 지속 시간(초). 0 이하이면 기본값(2초) 사용</param>
    public static void ShowToast(string message, float duration = 0f)
        => UIManager.Instance.ShowToast(message, duration);
